...
## 完了（横断検索 フェーズ1+2 / 2026-07-07）

- **フェーズ1（キーワード）**: `message_fts`（standalone FTS5・id UNINDEXED + content・trigram）を新設し、`messages` へのトリガ（`messages_fts_ai/ad/au`）で自動同期。多数の書き込み経路を手で触らずトリガに一元化。既存メッセージは `_init_db` でバックフィル（638件で検証）。
- **フェーズ2（意味）**: 既存 `memory_vec`（会話 Q&A ペア埋め込み・455件）を流用。**新規ベクトル化なし**で意味検索を実現。
- `store_search.py` の `search_messages()` がキーワード＋意味を **RRF で統合しセッション単位に集約**。キーワードヒットはジャンプ用 message_id を保持、スニペット生成。`GET /search/messages`（`routes/search.py`）。
- フロント: サイドバー検索を「タイトルフィルタ（即時）＋本文横断検索（2文字以上・debounce 300ms）」の2層に拡張。本文検索結果ブロック（スニペット＋語/意ソースバッジ）、クリックで該当ワークスペース＋セッションへジャンプ。
- 検証: 実データでキーワード・意味・RRF マージ・空クエリ高速パス・ルート登録・`npm run build` を確認。
...
- 検索フェーズ3（`message_vec`、保留）。
- 別構想: 生成メッセージのバージョン管理、ストーリー制作支援。
