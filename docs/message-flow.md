...
3. **記憶コンテキストを取得**（`memory_enabled=true` の場合）
   ...
   - ただし**現在のセッション自身が保存した記憶は検索対象から除外**する
   - ベクトル検索（`ruri-v3-310m` 埋め込み）とFTS5キーワード検索を組み合わせる
   - 関連する過去のQ&Aチャンクをシステムプロンプトに付加する

4. **補助コンテキストを結合**
   - 資料コンテキストと記憶コンテキストは別々の文字数上限で切り詰めてから結合する
   - デフォルトは資料 `2000` 文字、記憶 `1500` 文字で、設定から変更できる
...
**ファイル:** [src/renderer/stores/chatStore.ts](../src/renderer/stores/chatStore.ts) — `regenerateMessage()`  
**ファイル:** [backend/server.py](../backend/server.py) — `chat_regenerate_stream()`

再生成は通常送信と似ています。記憶検索のルールも同じで、**現在のセッション自身の記憶は常に除外**されます。

1. 対象の `user` メッセージと、その直後の `assistant` メッセージを特定する
2. モデルに渡す会話履歴は、その `user` メッセージまでに切り詰める
3. `memory_enabled=true` の場合は、現在セッションを除いたワークスペース記憶を検索して再生成用プロンプトを組み立てる
4. 生成完了後、既存の `assistant` メッセージを新しい内容で置き換える
5. 最後に、**そのセッション全体の記憶を再構築して保存し直す**

この挙動により、会話履歴にすでに含まれている内容を、同じセッションの記憶からもう一度注入してしまう不自然さを避けています。
再生成が終わったあとはセッション全体から記憶を作り直すため、以後ほかのセッションから検索されたときは最新の会話内容が反映されます。
