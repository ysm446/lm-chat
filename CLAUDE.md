...
UI に触れる変更（`src/renderer/**` の CSS・コンポーネント、新規画面や部品の追加）を行うときは、**必ず `docs/design/style-guide.md` を読み、そのルールに従う**。アプリ全体でアートスタイルを一貫させるための唯一の基準。

- 色・フォントサイズ・余白・角丸・モーション・z-index は、スタイルガイドのトークンと値の階段から選ぶ。コンポーネント側で生の hex や新しい寸法を発明しない。
- 新しい部品を作る前に、スタイルガイドの「部品」節と既存の同種部品を探し、同じトークンと寸法を再利用する。
- hover / active / focus / disabled / loading / drag の状態を必ず設計する。
- スタイルガイドから外れる必要がある場合は、理由を作業報告に書く。恒久的なルールになるなら、先に `docs/design/style-guide.md` を更新してから実装する。
- 新しいデザイントークンが必要なときは `src/renderer/styles.css` の `:root` に追加し、スタイルガイドのトークン表も更新する。
- スタイルガイドの「既知のズレ」に該当する箇所を触ったときは、その範囲だけ寄せてよい。無関係な箇所の一括整形はしない。
...
- チャット送信時に `build_document_context()` → `combine_contexts()` でワークスペース資料 3 チャンク・会話メモリ 5 チャンクを結合し、デフォルトは資料 2000 文字・記憶 1500 文字の個別上限で補助コンテキストを組み立てる
