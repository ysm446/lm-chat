...
if defined LM_CHAT_LLAMA_SERVER_EXE (
  ...
  for /f "usebackq delims=" %%i in (`powershell -NoLogo -NoProfile -Command "$pathsFile=Join-Path (Get-Location) 'data\llama_paths.json';if(Test-Path -LiteralPath $pathsFile){try{$paths=Get-Content -LiteralPath $pathsFile -Raw -Encoding UTF8|ConvertFrom-Json;$exe=[string]$paths.llama_exe;if($exe -and (Test-Path -LiteralPath $exe)){Write-Output $exe;exit 0}}catch{}};$roots=@((Join-Path (Get-Location) 'runtime\llama_cpp\versions'),(Join-Path (Get-Location) 'data\llama_cpp\versions'),(Join-Path (Get-Location) 'bin\llama-server'));$candidates=@();foreach($root in $roots){if(Test-Path -LiteralPath $root){$candidates+=Get-ChildItem -LiteralPath $root -Directory -ErrorAction SilentlyContinue|ForEach-Object{$exe=Join-Path $_.FullName 'llama-server.exe';if(Test-Path -LiteralPath $exe){$build=0;if($_.Name -match 'b(\d+)'){$build=[int64]$matches[1]};[pscustomobject]@{Build=$build;LastWriteTime=$_.LastWriteTime;Exe=$exe}}}}};$candidates=$candidates|Sort-Object -Property @{Expression='Build';Descending=$true},@{Expression='LastWriteTime';Descending=$true};if(-not $candidates){exit 1};$candidates[0].Exe"`) do set "LLAMA_SERVER_EXE=%%i"
)
