...
このファイルは、LM Chat で作業するエージェント向けのプロジェクトルールです。
...
- UI に触れる変更（`src/renderer/**` の CSS・コンポーネント、新規画面や部品の追加）を行うときは、**必ず `docs/design/style-guide.md` を読み、そのルールに従う**。
- 色・フォントサイズ・余白・角丸・モーション・z-index は、スタイルガイドで定義された値とトークンから選ぶ。生の hex や新しい寸法をコンポーネント側で発明しない。
